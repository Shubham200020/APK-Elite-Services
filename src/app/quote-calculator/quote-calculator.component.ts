...
@Component({
  ...
  template: `
    <section class="calculator-card">
      <div class="calc-header">
        <span class="calc-tag">Instant Rate Estimator</span>
        <h2>Calculate Your Cleaning Price</h2>
        <p>Select your home configuration and service details for an instant estimate range.</p>
      </div>

      <div class="calc-body">
        <!-- Step 1: Property Type -->
        <div class="calc-group">
          <label class="group-label">1. Property Size / Configuration</label>
        ...
        <div class="calc-group">
          <label class="group-label">3. Include Add-On Services (Optional)</label>
          <div class="addon-grid">
            ...
            <label class="addon-chip" [class.selected]="addons.balcony">
              ...
              <span>Balcony & Window Jet Wash (+₹399)</span>
            </label>
          </div>
        </div>
        ...
        <div class="result-box">
          <div class="estimate-text">
            ...
            <span class="est-note">*Final quote verified on-site or via WhatsApp consultation</span>
          </div>
  ...
  styles: [
    ...
    `.calculator-card { background: white; border-radius: 16px; border: 1px solid #e2e8f0; padding: 2rem; box-shadow: 0 4px 20px rgba(0,0,0,0.03); max-width: 800px; margin: 0 auto; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }`,
    `.calc-header { text-align: center; margin-bottom: 1.75rem; }`,
    `.calc-tag { display: inline-block; font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; color: #0284c7; background: #f0f9ff; border: 1px solid #bae6fd; padding: 0.25rem 0.65rem; border-radius: 6px; margin-bottom: 0.5rem; }`,
    `h2 { font-size: 1.5rem; color: #0f172a; margin: 0 0 0.4rem; font-weight: 700; letter-spacing: -0.01em; }`,
    ...
    `.options-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 0.65rem; }`,
    `.options-grid.two-col { grid-template-columns: 1fr 1fr; }`,
    `.option-btn { padding: 0.65rem 0.5rem; border: 1px solid #cbd5e1; background: #ffffff; border-radius: 8px; font-size: 0.88rem; font-weight: 600; color: #334155; cursor: pointer; transition: all 0.15s ease; text-align: center; }`,
    `.option-btn:hover { border-color: #0284c7; background: #f0f9ff; }`,
    `.option-btn.active { border-color: #0284c7; background: #0284c7; color: white; }`,
    `.addon-grid { display: flex; flex-wrap: wrap; gap: 0.65rem; }`,
    `.addon-chip { display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.5rem 0.85rem; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.85rem; font-weight: 500; color: #334155; cursor: pointer; background: white; transition: all 0.15s ease; }`,
    `.addon-chip.selected { border-color: #16a34a; background: #f0fdf4; color: #15803d; font-weight: 600; }`,
    `.addon-chip input { cursor: pointer; }`,
    `.result-box { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 1.5rem; margin-top: 1.5rem; display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap; }`,
    `.estimate-text { display: flex; flex-direction: column; }`,
    `.est-label { font-size: 0.8rem; color: #64748b; font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; }`,
    `.est-price { font-size: 1.6rem; font-weight: 800; color: #0f172a; letter-spacing: -0.02em; }`,
    `.est-note { font-size: 0.75rem; color: #94a3b8; }`,
